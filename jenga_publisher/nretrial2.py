...
class JengaBlockDetector(Node):
    ...
    def world_coordinates(self, scale_factor, obb_xyxyxyxy):
        ...
        centroid_x_pixel = (obb_xyxyxyxy[0][3][0] + obb_xyxyxyxy[0][0][0] + obb_xyxyxyxy[0][2][0] + obb_xyxyxyxy[0][1][0])  / 4
        centroid_y_pixel = (obb_xyxyxyxy[0][0][1] + obb_xyxyxyxy[0][1][1] + obb_xyxyxyxy[0][2][1] + obb_xyxyxyxy[0][3][1]) / 4

        p_t = np.array([centroid_x_pixel, centroid_y_pixel, 1])
        p_c_t = np.matmul(np.linalg.inv(self.camera_matrix), p_t)
        p_c_t = scale_factor * p_c_t

        p_w_tT = np.matmul(self.w_R_c, np.array([p_c_t[0], p_c_t[1], p_c_t[2],1]))
        p_w_t = np.array([p_w_tT[1], p_w_tT[0], p_w_tT[2], p_w_tT[3]])
        ...
        msg.data = [-p_w_t[0], p_w_t[1], -p_w_t[2], theta_degrees]
